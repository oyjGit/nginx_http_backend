...
template<typename T>
void test(const T& t, std::string& json) 
{
	slothjson::encode <false, T>(t, json);
}
...
int main(int argc, char * argv[])
{
	...
	CLoginBackParam enc;
	std::string encJson;
	//slothjson::encode<true, CLoginBackParam>(enc, encJson);
	slothjson::perf_object_t perf;
	test<slothjson::perf_object_t>(perf, encJson);


	//CLoginBackParam login;
	//std::string json = "{\"code\":1, \"desc\":\"desc value\", \"data\":\"123\"}";
	//slothjson::decode<CLoginBackParam>(json, login);
	//login.print();
    //slothjson::test();
	getchar();
    return 0;
